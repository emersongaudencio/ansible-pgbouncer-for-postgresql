...
if [ "$VAR_PG_VERSION" -gt 0 -a "$VAR_HOST" != "" ]; then
  ### Ping host ####
  ansible -i $SCRIPT_PATH/hosts -m ping $VAR_HOST -o

  ### PgBouncer Setup ####
  ansible-playbook -v -i $SCRIPT_PATH/hosts -e "{postgresql_version: '$VAR_PG_VERSION'}" $SCRIPT_PATH/playbook/pgbouncer.yml -l $VAR_HOST
else
